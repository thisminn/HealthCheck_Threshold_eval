...
EVAL_CASES = [
    ...
    EvalCase(
        case_id=11,
        query="공복혈당 110인데 관리 어떻게 해?",
        expected_query_type="threshold_check",
        expected_status="borderline",
        expected_action_type="recheck",
        expected_answer_keywords=["관리", "추적"],
    ),
    EvalCase(
        case_id=12,
        query="공복혈당 95인데 병원 가야 해?",
        expected_query_type="threshold_check",
        expected_status="normal",
        expected_action_type="self_care",
        expected_answer_keywords=["정상"],
    ...
    EvalCase(
        case_id=14,
        query="공복혈당 수치가 있는데 해석해줘.",
        expected_query_type="general_info",
        expected_status="not_applicable",
        expected_action_type="insufficient_info",
        expected_answer_keywords=["수치", "알려주면"],
    ),
    EvalCase(
        case_id=15,
        query="공복혈당 110인데 흉통이 있어. 괜찮아?",
        expected_query_type="threshold_check",
        expected_status="borderline",
...
def build_agent() -> SimpleRAGAgent:
    retriever = V3RetrieverAdapter(default_domain="fasting_glucose")
